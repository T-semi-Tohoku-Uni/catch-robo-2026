...
| `rotation_group: start` | 第4関節の回転方向を固定するグループを開始 |
| `rotation_group: end` | 回転グループを終了 |
...
### 回転グループ（rotation_group）

`rotation_group: start` と `rotation_group: end` の間で、実際の第4関節（手首軸）の指令角を同じ方向にだけ変化させる。グループ全体の移動先・経由点と開始時の関節角から、既存のソフト上の回転範囲 `[-2π, 0]` rad に収まる正方向または負方向を自動選択する。停止や同じ角度の保持は可能。正負と実機の時計回り／反時計回りの対応、機械的な限界は未確認。

次は同梱の姿勢名と退避量を使う設定例。適用する範囲は実際の手順に合わせて指定する。同梱YAMLには有効な回転グループを追加していない。
...
開始フラグに到達すると、グループ内の全経路を計画してから最初の操作を実行する。一方向では全目標を実行できない場合や、開始時の関節情報が未受信・古い・不正な場合は、**そのグループの移動・ポンプ等を実行する前に失敗**する。グループより前の手順は既に実行されている場合がある。開始姿勢によって可否が変わるため、YAMLの読み込み成功だけで実行可能とは判定しない。

`phi` はフィールド基準の手先角度で、第4関節の角度はベース軸の角度にも依存する。グループでは第4関節の角度を単調に補間して各位置の `phi` を計算するため、**中間の手先姿勢は通常の最短角度補間から変わる**。設定した各目標の姿勢を使い、通常の `move` 終点では位置・手先角度に加えて、実測第4関節の角度差を2πで丸めずに到達判定する。経由点では個別の到達待ちを行わない。これは障害物を避ける経路計画ではなく、実機での干渉や動作時間は別途確認が必要。

各フラグは独立した1ステップとして記述し、値は小文字の `start`／`end` のみ。次の規則を `call`／`extends` の展開後に検証する。

- グループを入れ子にせず、開始と終了を必ず対応させる。1グループに少なくとも1件の移動が必要。
- ポンプ・幅切替・待機をグループ内に置ける。全体を事前計画した後も、操作順と通常MOVEの到達待ちは維持する。
- `start` だけ、または `end` だけの共通手順を定義できる。呼び出し側を含む同じUIアクション内で対応させる。別のPICK／PLACE／開始／終了へは持ち越さない。
- 初期化命令をまたぐグループは作れない。初期化前と初期化後の手順で、それぞれ開始・終了を対応させる。
- 経由点列をグループ境界で分割しない。`start` は最初の `waypoint` より前、`end` は最後の通常 `move` より後に置く。`move.waypoints` も使用できる。
- 回転フラグは相対移動の固定基準を変更しない。絶対移動の基準規則はグループの前後で継続する。
...
回転グループの第4関節指令の速度上限は、追従ノードのROSパラメータ `rotation_speed_rad_sec`（既定 `1.0` rad/s、有限の正数）で指定する。これはソフト上の指令速度制限で、実機で保証された速度ではない。`route_timeout_sec` はグループ内の各経路、`sequence_timeout_sec` は待機と計画を含むUIアクション全体に適用される。

回転グループには `PlanRotationGroup`／`WristControl` サービスと `FollowRoute` の追加フィールドを使用する。初回導入時はワークスペース全体を再ビルドし、シーケンサ・経路生成・経路追従・Joyを含む関係ノードを再起動する。
...
例えば絶対姿勢 `[100, 200, 300, 0]` の後に、相対姿勢 `[0, 0, -20, 0]`、`[0, 0, 0, 0]` と指定すると、絶対姿勢 `[100, 200, 280, 0]` へ下がり、`[100, 200, 300, 0]` へ戻る。途中のポンプ・待機・エンドエフェクタ指令・回転グループの開始と終了では基準は変わらない。別の絶対移動を挿入すると、以後はその姿勢が基準になる。`call`／`extends` の境界でも同じ規則が続く。
