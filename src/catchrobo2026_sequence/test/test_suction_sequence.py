...
from catchrobo2026_msgs.srv import (
    CheckSuction, EndeffectorControl, GenerateRoute, PlanRotationGroup, PumpControl, WristControl,
)
...
class Harness:
    def __init__(self, node, executor, tmp_path, log):
        ...
        self.route_requests = []
        self.group_plans, self.wrist_operations, self.follow_goals = [], [], []
        ...
        self.services = [
            ...
            node.create_service(CheckSuction, 'check_suction', self.check,
                                callback_group=self.group),
            node.create_service(PlanRotationGroup, 'plan_rotation_group', self.plan_group,
                                callback_group=self.group),
            node.create_service(WristControl, 'wrist_control', self.wrist,
                                callback_group=self.group),
        ]
    ...
    def follow(self, goal):
        ...
        self.follow_goals.append(goal.request)
        goal.succeed()
        return FollowRoute.Result(success=True)

    def plan_group(self, request, response):
        self.events.append(('group_plan', len(self.group_plans)))
        self.group_plans.append(request)
        begin = 0
        for end in request.route_ends:
            poses = [PoseStamped(pose=pose) for pose in request.targets[begin:end]]
            response.routes.append(RotationGroupRoute(
                path=RosPath(poses=poses), wrist_angles=[0.0] * len(poses),
                phi_angles=[0.0] * len(poses)))
            begin = end
        response.direction = 0 if request.allow_wrist_reversal else 1
        response.interval_phi_travel = [0.0] * len(request.phi_travel_intervals)
        response.success = True
        return response

    def wrist(self, request, response):
        self.events.append(('wrist', request.operation))
        self.wrist_operations.append(request)
        response.success = True
        return response
...
def test_pick_retries_break_on_success_and_fail_at_limit(rig, outcomes, attempts, success):
    ...
    assert rig.routes == [(100, 200, 300, 0)] + [
        (100, 200, 270, 0), (100, 190, 270, 0), (100, 200, 300, 0)] * attempts
    ...
    commands = [event for event in rig.events if event[0] in ('move', 'pump', 'endeffector')]
    for index, event in enumerate(commands):
        if event[0] == 'pump':
            assert commands[index - 1] == ('move', (100, 190, 270, 0))
            assert commands[index + 1] == ('move', (100, 200, 300, 0))
...
def test_monitor_runs_while_pump_and_routes_execute_then_waits(rig):
    ...
    _, pending = rig.start()
    rig.until(lambda: len(rig.routes) == 4 and len(rig.pumps) == 1)
    rig.observe(0.1)
...
def test_suction_retries_consume_pending_waypoint_once_and_clear_it_after_failure(rig):
    rig.check_replies.extend(CheckReply(value, timed_out=not value) for value in [
        False, True, False, False, False, True,
    ])
    rig.launch([
        *retry_steps(),
        {'waypoint': {'absolute': [500, 600, 700, 0]}},
    ])
    _, first = rig.start()
    result = rig.resolve(first)
    assert result.result.success, result.result.message
    assert '1 waypoints deferred' in result.result.message
    assert len(rig.routes) == 7
    assert all(not request.waypoints for request in rig.route_requests)

    _, second = rig.start()
    result = rig.resolve(second)
    assert not result.result.success and result.status == GoalStatus.STATUS_ABORTED
    assert 'suction retry limit reached' in result.result.message
    assert len(rig.routes) == 17
    continuation = rig.route_requests[7]
    assert continuation.use_explicit_waypoints
    assert len(continuation.waypoints) == 1
    position = continuation.waypoints[0].position
    assert (position.x, position.y, position.z) == pytest.approx((0.5, 0.6, 0.7))
    assert all(not request.waypoints for request in rig.route_requests[8:])

    _, third = rig.start()
    result = rig.resolve(third)
    assert result.result.success, result.result.message
    assert len(rig.routes) == 21
    assert all(not request.waypoints for request in rig.route_requests[17:])
    assert len(rig.checks) == 6 and rig.widths == [1, 1]


@pytest.mark.parametrize('experimental', [False, True])
def test_builtin_pick_keeps_configured_moves_with_optional_suction_retries(rig, experimental):
    ...
    correction = (above[0], above[1] - 10, approach[2], above[3])
    ...
    if experimental:
        ...
        config['sequences']['pick_common']['steps'] = [
            {'call': 'pick_with_suction_check'},
        ]
        config_file = rig.tmp_path / 'experimental.yaml'
        config_file.write_text(yaml.safe_dump(config))
    else:
        rig.node.destroy_service(rig.services[3])
    rig.launch_file(config_file)
    _, pending = rig.start(mask=3)
    result = rig.resolve(pending)
    assert result.result.success, result.result.message
    attempts = 2 if experimental else 1
    assert rig.checks == ([(3, timeout)] * attempts if experimental else [])
    assert rig.pumps == [(1, 1, 0)] * attempts
    assert rig.routes == [above] + [approach, correction, retreat] * attempts
...
def test_real_pump_sensor_timeout_then_selected_collectors_succeed(rig):
    ...
    _, pending = rig.start(mask=5)
    rig.until(lambda: len(rig.routes) == 5)
    assert not pending.done()
    readings[:] = [0x60, 0, 0x60]
    result = rig.resolve(pending)
    assert result.result.success, result.result.message
    assert len(rig.routes) == 7 and rig.widths == [1]
...
def test_suction_monitor_preserves_group_waypoints_and_phi_intervals(rig):
    rig.check_replies.append(CheckReply(True, hold=True))
    rig.launch([
        {'suction_check': {'start': {'timeout': 3.0}}},
        {'sequence_group': {'start': True, 'max_phi_travel': 0.5}},
        {'waypoint': {'absolute': [100, 200, 300, 0]}},
        {'move': {'absolute': [120, 200, 300, 0]}},
        {'phi_travel': {'start': True, 'limit': 0.3}},
        {'move': {'relative': [0, 0, -30, 0]}},
        {'phi_travel': 'end'},
        {'pump': 'suction'},
        {'suction_check': 'wait'},
    ...
    _, pending = rig.start(mask=5)
    rig.until(lambda: len(rig.follow_goals) == 2 and len(rig.pumps) == 1)
    rig.observe()
    assert not pending.done() and rig.widths == []
    assert rig.routes == []
    assert len(rig.group_plans) == 1
    plan = rig.group_plans[0]
    assert list(plan.route_ends) == [2, 3]
    assert [pose.position.z for pose in plan.targets] == pytest.approx([0.3, 0.3, 0.27])
    assert plan.allow_wrist_reversal and plan.limit_phi_travel
    assert plan.max_phi_travel == 0.5
    assert len(plan.phi_travel_intervals) == 1
    interval = plan.phi_travel_intervals[0]
    assert (interval.start_target, interval.end_target, interval.max_phi_travel) == (2, 3, 0.3)
    rig.release(0)
    assert rig.resolve(pending).result.success
    assert [request.operation for request in rig.wrist_operations] == [
        WristControl.Request.BEGIN, WristControl.Request.PHI_BEGIN,
        WristControl.Request.PHI_END, WristControl.Request.END,
    ]
    group_id = rig.wrist_operations[0].group_id
    assert group_id != 0
    assert all(request.group_id == group_id for request in rig.wrist_operations)
    assert all(goal.rotation_group_id == group_id for goal in rig.follow_goals)
    assert rig.pumps == [(1, 0, 1)] and rig.widths == [1]


def test_cancel_during_group_suction_wait_releases_wrist_and_ignores_late_reply(rig):
    rig.check_replies.extend([
        CheckReply(True, hold=True), CheckReply(False, timed_out=True, hold=True),
    ])
    rig.launch([
        {'sequence_group': {'start': True}},
        {'move': {'absolute': [100, 200, 300, 0]}},
        {'suction_check': {'start': {'timeout': 3.0}}},
        {'pump': 'suction'},
        {'suction_check': 'wait'},
    ...
    assert [request.operation for request in rig.wrist_operations] == [
        WristControl.Request.BEGIN, WristControl.Request.END,
    ]
    first_group_id = rig.wrist_operations[0].group_id
    assert rig.wrist_operations[1].group_id == first_group_id
    assert rig.replied == [] and rig.widths == []
    ...
    assert len(rig.wrist_operations) == 3
    rig.release(1)
    ...
    assert rig.wrist_operations[2].group_id == rig.wrist_operations[3].group_id
    assert rig.wrist_operations[2].group_id != first_group_id
