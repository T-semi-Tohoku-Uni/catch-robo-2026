...
吸引判定・条件分岐・最大回数付き再試行も利用できます。同梱設定は通常PICKを既定とし、`pick_common.steps` の `pick_without_suction_check` と `pick_with_suction_check` のコメントを入れ替えると切り替わります。実験側も現在の下降・Y補正・吸引・退避を共有します。通常へ戻すときは実験側をコメントアウトし、通常側を有効にします。比較方向とセンサー対応を設定してから使用してください。[切替方法と制約](CONFIG.md#実験用吸引自動化の切り替え) を参照してください。
